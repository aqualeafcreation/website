...
const WHATSAPP_NUMBER = '917028380231' // country code + number, no +

function handleFormSubmit(e) {
  e.preventDefault()
  const name    = e.target.name.value.trim()
  const city    = e.target.city.value.trim()
  const space   = e.target.space.value.trim()
  const message = e.target.message.value.trim()

  const text = encodeURIComponent(
    `Hello AquaLeaf Creation 🌿\n\n` +
    `Name: ${name}\n` +
    `City: ${city}\n` +
    `Space Type: ${space}\n` +
    `Message: ${message}\n\n` +
    `- Sent via aqualeafcreation.in`
  )
    ...
    <>
      ...
      <footer className={styles.footer} id="contact">
        <div className={styles.formSection}>
          ...
          <form className={styles.form} onSubmit={handleFormSubmit} noValidate>
            <div className={styles.formRow}>
              ...
              <div className={styles.formGroup}>
                <label className={styles.label} htmlFor="f-city">City</label>
                <input
                  id="f-city"
                  name="city"
                  type="text"
            ...
            <div className={styles.formGroup}>
              <label className={styles.label} htmlFor="f-space">Space Type</label>
              <input
                id="f-space"
                name="space"
                type="text"
                placeholder="e.g. Penthouse Living Room, Office Lobby"
                className={styles.input}
              />
            </div>

            <div className={styles.formGroup}>
              <label className={styles.label} htmlFor="f-message">Your Vision</label>
              <textarea
                id="f-message"
                name="message"
                rows={3}
        ...
        <div className={styles.copy}>
          <p>© 2026 AquaLeaf Creation. All Rights Reserved.</p>
        </div>
